SELECT 
	COUNT(locationName) AS 'Registered',
	(Case when (select count(locationName) from shelter.Location where locationName IS NOT NULL
	AND congregationId IN (select id from organization.Congregation where timeRetired IS NULL)) != 0 then
	(COUNT(locationName) * 100 /  (select count(locationName) from shelter.Location where locationName IS NOT NULL
	AND congregationId IN (select id from organization.Congregation where timeRetired IS NULL))) /*Replace with real total later*/
	else 0 end
	) AS 'Percentage'
FROM
(SELECT 
