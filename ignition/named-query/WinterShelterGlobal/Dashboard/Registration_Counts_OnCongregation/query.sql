SELECT 
	...
	(Case when (select count(locationName) from shelter.Location) != 0 then
	(COUNT('locationName') * 100 /  (select count(locationName) from shelter.Location)) /*Replace with real total later*/ 
	else 0 end
	) AS 'Percentage'
FROM(
SELECT 
