...
-- Get average min max beds per dayOfWeek
SELECT
	s.id
	,s.dayOfYear 
	--,DateAdd(DAY,s.dayOfYear,DateFromParts(Year(GetDate())-1,12,31)) as DateFromDayOfYear
	,DateAdd(DAY,dayOfYear, case when dayOfYear<@firstDayOfTheCurrentSeason then DateFromParts(@YeareEnd-1,12,31) Else DateFromParts(@YearStart-1,12,31) end) as DateFromDayOfYear
	,DATEPART(WEEKDAY,DateAdd(DAY,dayOfYear, case when dayOfYear<@firstDayOfTheCurrentSeason then DateFromParts(@YeareEnd-1,12,31) Else DateFromParts(@YearStart-1,12,31) end)) as DayOfWeek
	,DATENAME(WEEKDAY,DateAdd(DAY,dayOfYear, case when dayOfYear<@firstDayOfTheCurrentSeason then DateFromParts(@YeareEnd-1,12,31) Else DateFromParts(@YearStart-1,12,31) end)) as DayName
	,s.reservedBeds
	,s.genderId
	,s.totalBeds
	,s.seasonId
...
WHERE
	...
	AND locationId in (SELECT * FROM @registrationTable)
order by DayName